...
if(isset($_POST['submitupload'])){
    ...
    if(in_array($filecheck, $fileextstored)){
        ...
        $q = "INSERT INTO `tbl_newvisitors`(`visitorname`, `phonenumber`, `address`, `postcode`, `state`,`city`,`inasis`,`date`, `upload_pic`) 
        VALUES ('$visitorname','$phonenumber','$address ','$postcode','$state','$city','$inasis','$date','$destinationfile')";

        $query = mysqli_query($con, $q);

        if($query){
            $_SESSION['status'] = "New Visitor Registration Successfully Added!";
            header("Location: /FYP2/php/addNewRegistration.php");
        }
        else{
